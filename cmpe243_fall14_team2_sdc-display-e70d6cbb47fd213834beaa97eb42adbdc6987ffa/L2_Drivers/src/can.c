...
static uint32_t CAN_MSG_COUNT=0;
 ...
 uint32_t GetCanMsgCount()
{
 return CAN_MSG_COUNT;
}
...
void CAN_bypass_filter_ack_all_msgs()
{
...
can_msg_t CreateCANMessage(const uint32_t nFrame,const uint32_t nMsg_ID,const bool bIs29,const uint8_t nData_len,const char* pWords)
{
                can_msg_t msg;
                memset(&msg, 0, sizeof(msg));
                msg.frame = nFrame;
                msg.msg_id = nMsg_ID;
                msg.frame_fields.is_29bit = bIs29;
                msg.frame_fields.data_len = nData_len;
                msg.data.bytes[0]=pWords[0];
                msg.data.bytes[1]=pWords[1];
                msg.data.bytes[2]=pWords[2];
                msg.data.bytes[3]=pWords[3];
                msg.data.bytes[4]=pWords[4];
                msg.data.bytes[5]=pWords[5];
                msg.data.bytes[6]=pWords[6];
                msg.data.bytes[7]=pWords[7];

                return msg;
}


void CAN_test_bufoff_cb(uint32_t i)
{
    puts("CB: BUS OFF");
}
void CAN_test_bufovr_cb(uint32_t i)
{
    puts("CB: DATA OVR");
}
...
void InitializeCan()
{
    if(!CAN_init(can1, 100, 0, 0, CAN_test_bufoff_cb, CAN_test_bufovr_cb))
...
void SetCANFilter(const uint32_t can29_FilterStart,const uint32_t cant29_FilterStop)
{
    const can_ext_grp_id_t eglist[] = { {CAN_gen_eid(can1, can29_FilterStart), CAN_gen_eid(can1, cant29_FilterStop)} }; // Group 1

    printf("  Test filter setup\n");

    CAN_setup_filter(0, 0, 0, 0, 0, 0, eglist, 1);



    const uint8_t offset = 4;
    LPC_CANAF->SFF_sa     == 4  - offset;
    LPC_CANAF->SFF_GRP_sa == 12 - offset;
    LPC_CANAF->EFF_sa     == 20 - offset;
    LPC_CANAF->EFF_GRP_sa == 28 - offset;
    LPC_CANAF->ENDofTable == 36 - offset;
    int i=0;
    for (; i < 10; i++) {
